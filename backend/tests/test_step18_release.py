...
def test_release_constants():
    ...
    assert RELEASE_BUILD == "12"
    assert RELEASE_TAG == "v0.3.1"
    assert RELEASE_LABEL == "v0.3.1 (build 12)"
    assert ELECTRON_ARTIFACT == "SSC-Setup-0.3.1.exe"
    assert ANDROID_ARTIFACT == "SSC-0.3.1.apk"
    assert ELECTRON_CLIENT_HEADER == "electron/0.3.1/12"
    assert ANDROID_CLIENT_HEADER == "android/0.3.1/12"

...
foreach ($check in $legacyElectronChecks) {
    if (-not (Test-Path $check.Path)) {
        Write-Host "WARN: Phase 2 legacy Electron check skipped (missing): $($check.Path)"
        continue
    }
    $text = Get-Content $check.Path -Raw
    if ($text -notmatch $check.Pattern) {
        Write-Error "Phase 2 legacy Electron policy: $($check.Path) must contain $($check.Pattern)"
    }
}
